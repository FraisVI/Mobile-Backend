...
@section('content')
    ...
    <a href="{{ route('stories.create') }}" class="btn btn-dark mb-3">
    ...
    <table id="stories-table" class="table">
        ...
        <?php /** @var \App\Models\Stories $stories */ ?>
        @foreach($stories as $s)
            <tr>
                ...
                <td><img src="{{ URLHelper::transform($s->thumb) }}" /></td>
                <td>
                    {{ $s->title }} <br />
                ...
                <td>{{ $s->published == 1 ? '+' : '' }}</td>
                <td>
                    <form onSubmit="if(!confirm('Вы уверены, что хотите удалить запись?')){return false;}" action="{{ route('stories.destroy', [$s->id]) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <div class="btn-group" role="group">
                            <a href="{{ route('stories.edit', [$s->id]) }}" class="btn btn-primary btn-sm">Изменить</a>
